...
//取出记录总数
$totalsql="select * from docs where typeid=$typeid";
...
$sql="select * from docs where typeid=$typeid order by id desc limit ".$page_size*($pageCurrent-1).",".$page_size;
...
?>
<div class="greenLocation">您当前的位置>各润资讯><?php ?>></div>
...
<ul>
	<?php while($row=$result->fetch_assoc() ){
		...
		?> 
	<li class="newsentry"><a href="docs.php?id=<?php echo $id; ?>"><?php echo $title; ?></a>
		<span class="newslist_time">下载：<?php echo $hits; ?>  发布时间：<?php echo $time; ?></span>
	
	</li>
	<?php } ?>
</ul>
<p class="pagination">本分类下<?php $subPages=new SubPages($page_size,$nums,$pageCurrent,$sub_pages,"doclist.php?typeid=$typeid&page=",1);?></p>
</div>
<div class="greenNewsright">
<div class="newsType">
	<h2>新闻分类</h2>
	<ul>
		<li><a href="doclist.php?typeid=4">数学资料</a></li>
		<li><a href="doclist.php?typeid=5">物理资料</a></li>
		<li><a href="doclist.php?typeid=6">化学资料</a></li>
		<li><a href="doclist.php?typeid=7">英语资料</a></li>
		<li><a href="doclist.php?typeid=8">语文资料</a></li>
	</ul>
</div>
...
<?php 
//取出点击最多的十条新闻
         $sqltop="select * from docs  order by hits desc limit 10";
		...
		{ 
			...
			$topid=$row_top['id'];
?>
		<li><a href="docs.php?id=<?php echo $topid ?>" title="<?php echo $toptitle ?>"><?php echo cut_str($toptitle,16,0) ; ?></a></li>
<?php } ?>
	</ul>
</div>
